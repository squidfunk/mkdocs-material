...
organized.
  ...
  - __Limitations of automated tools:__ While we understand that automated tools(such as Lighthouse, Accessibility tools, etc.), might seem to help to
  identify potential issues, simply submitting a report generated by these tools does not constitute a complete bug report. These tools often contain verbose outputs as well as false positives, which necessitate a critical evaluation. You can additionally attach such reports to your issue; however, this does not substitute the requirement for a minimal reproduction or a thorough discussion of the findings. _We reserve the right to mark these issues as incomplete and close them._ This practice ensures that we are addressing genuine concerns
